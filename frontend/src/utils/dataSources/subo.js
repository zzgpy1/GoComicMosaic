...
const CONFIG = {
  // 基础URL - 直接设置为完整的API地址
  baseUrl: 'https://subocaiji.com/api.php/provide/vod/from/subm3u8/at/json', // JSON格式
  //baseUrl: 'https://subocaiji.com/api.php/provide/vod/at/xml/', // XML格式
  
  // 是否使用XML格式 (true: XML格式, false: JSON格式)
  useXml: false
};

const suboDataSource = {
  // 基本信息
  name: '速播',
  description: '速播影视API，提供电影、电视剧、动漫、综艺等内容',
  ...
  parseResponse(response) {
    ...
    try {
      ...
      console.error('解析速播API响应失败:', error);
      return null;
    }
  },
  ...
  async searchMovies(keyword, page = 1, pageSize = 12) {
    try {
      ...
      if (jsonData && jsonData.list && Array.isArray(jsonData.list)) {
        console.log('速播API搜索结果:', jsonData);
      ...
      console.error('速播API搜索失败:', error);
      throw new Error(`速播搜索失败: ${error.message}`);
    }
  },
  ...
  async getMovieDetail(id) {
    try {
      ...
      console.error('速播API获取详情失败:', error);
      throw new Error(`速播获取详情失败: ${error.message}`);
    }
  }
};

export default suboDataSource; 