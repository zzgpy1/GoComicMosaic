...
const CONFIG = {
  // 基础URL - 直接设置为完整的API地址
  baseUrl: 'https://cj.lziapi.com/api.php/provide/vod/from/lzm3u8/at/xml/', // XML格式
  // 是否使用XML格式 (true: XML格式, false: JSON格式)
  useXml: true
};

const liangziDataSource = {
  // 基本信息
  name: '量子',
  description: '量子影视API，提供电影、电视剧、动漫、综艺等内容',
  ...
  parseResponse(response) {
    ...
    try {
      ...
      console.error('解析量子API响应失败:', error);
      return null;
    }
  },
  ...
  async searchMovies(keyword, page = 1, pageSize = 12) {
    try {
      ...
      if (jsonData && jsonData.list && Array.isArray(jsonData.list)) {
        console.log('量子API搜索结果:', jsonData);
        
        return {
          dataList: jsonData.list.map(item => ({
            vod_id: item.vod_id,
            vod_name: item.vod_name,
            vod_blurb: item.vod_content || '', // XML中是des字段
      ...
      console.error('量子API搜索失败:', error);
      throw new Error(`量子搜索失败: ${error.message}`);
    }
  },
  ...
  async getMovieDetail(id) {
    try {
      ...
      console.error('量子API获取详情失败:', error);
      throw new Error(`量子获取详情失败: ${error.message}`);
    }
  }
};

export default liangziDataSource; 