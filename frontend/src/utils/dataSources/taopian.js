/**
 * 数据源模板文件 (同时支持JSON和XML格式)
 * 
 * 使用方法:
 * 1. 复制此文件并重命名为 yourDataSourceName.js
 * 2. 修改下面的配置和实现
 * 3. 系统将自动加载你的数据源
 */
...
const CONFIG = {
  // 基础URL - 直接设置为完整的API地址
  // JSON格式示例: 'https://api.example.com/api.php/provide/vod/'
  // XML格式示例: 'https://api.example.com/api.php/provide/vod/at/xml/'
...
const taopianDataSource = {
  // 【必填】数据源显示名称
  name: '淘片',
  
  // 【必填】数据源简要描述
  description: '淘片影视API，提供电影、电视剧、动漫、综艺等内容',
  
  // 【必填】API基础URL - 使用配置中的URL
  get baseUrl() {
    return CONFIG.baseUrl;
  },
  
  // 【必填】创建axios实例 - 根据配置动态生成
  get apiClient() {
      ...
      if (jsonData && jsonData.list && Array.isArray(jsonData.list)) {
        ...
        return {
          dataList: jsonData.list.map(item => ({
            ...
            vod_blurb: item.vod_content || item.vod_blurb || '', // 支持不同字段名
            vod_pic: item.vod_pic || '',
            vod_remarks: item.vod_remarks || item.vod_note || '', // 支持不同字段名
...
export default taopianDataSource; 