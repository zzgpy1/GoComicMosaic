...
const CONFIG = {
  // 基础URL - 直接设置为完整的API地址
  baseUrl: 'https://ikunzy.vip/api.php/provide/vod/', // JSON格式
  //baseUrl: 'https://ikunzy.vip/api.php/provide/vod/at/xml/', // XML格式
  
  // 是否使用XML格式 (true: XML格式, false: JSON格式)
  useXml: false
};

const aikunDataSource = {
  // 基本信息
  name: '爱坤',
  description: '爱坤影视API，提供电影、电视剧、动漫、综艺等内容',
  ...
  parseResponse(response) {
    ...
    try {
      ...
      console.error('解析爱坤API响应失败:', error);
      return null;
    }
  },
  ...
  async searchMovies(keyword, page = 1, pageSize = 12) {
    try {
      ...
      if (jsonData && jsonData.list && Array.isArray(jsonData.list)) {
        console.log('爱坤API搜索结果:', jsonData);
      ...
      console.error('爱坤API搜索失败:', error);
      throw new Error(`爱坤搜索失败: ${error.message}`);
    }
  },
  ...
  async getMovieDetail(id) {
    try {
      ...
      console.error('爱坤API获取详情失败:', error);
      throw new Error(`爱坤获取详情失败: ${error.message}`);
    }
  }
};

export default aikunDataSource; 