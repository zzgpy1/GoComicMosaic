...
const CONFIG = {
  // 基础URL - 直接设置为完整的API地址
  baseUrl: 'https://jszyapi.com/api.php/provide/vod/from/jsm3u8/at/json', // JSON格式
  //baseUrl: 'https://jszyapi.com/api.php/provide/vod/at/xml/', // XML格式
  
  // 是否使用XML格式 (true: XML格式, false: JSON格式)
  useXml: false
};

const jisuDataSource = {
  // 基本信息
  name: '极速',
  description: '极速影视API，提供电影、电视剧、动漫、综艺等内容',
  ...
  parseResponse(response) {
    ...
    try {
      ...
      console.error('解析极速API响应失败:', error);
      return null;
    }
  },
  ...
  async searchMovies(keyword, page = 1, pageSize = 12) {
    try {
      ...
      if (jsonData && jsonData.list && Array.isArray(jsonData.list)) {
        console.log('极速API搜索结果:', jsonData);
      ...
      console.error('极速API搜索失败:', error);
      throw new Error(`极速搜索失败: ${error.message}`);
    }
  },
  ...
  async getMovieDetail(id) {
    try {
      ...
      console.error('极速API获取详情失败:', error);
      throw new Error(`极速获取详情失败: ${error.message}`);
    }
  }
};

export default jisuDataSource; 